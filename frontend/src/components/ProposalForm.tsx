...
import { formatCurrency } from '../utils/proposalDisplay';
      ...
      {result && (
        <div className={`mt-4 rounded-lg border p-4 ${result.status === 'APPROVED' ? 'border-emerald-800 bg-emerald-950/40' : 'border-red-800 bg-red-950/40'}`}>
          ...
          <div className="mt-3 space-y-2 text-sm text-slate-300">
            <p>Oferta: <span className="text-white">Oferta {tipoOferta}</span></p>
            <p>
              Renda: {formatCurrency(Number(renda))} | Investimentos: {formatCurrency(Number(investimentos))} | Tempo: {tempoContaAnos} ano(s)
            </p>
            {result.status === 'APPROVED' ? (
              <>
                <p className="text-emerald-300">Beneficios aprovados e ativados:</p>
                <BenefitBadges benefits={result.activatedBenefits} emptyLabel="Nenhum beneficio selecionado" />
                {result.cardAccount.accountId && (
                  <p className="text-slate-400">Conta cartao: <span className="text-white">{result.cardAccount.accountId}</span></p>
                )}
              </>
            ) : (
              <>
                <p className="text-red-300">Beneficios solicitados:</p>
                <BenefitBadges benefits={beneficios} emptyLabel="Nenhum" />
              </>
            )}
          </div>
          {result.motivosRejeicao.map((m) => <p key={m} className="mt-2 text-sm text-red-300">{m}</p>)}
        </div>
      )}
      {execution && (

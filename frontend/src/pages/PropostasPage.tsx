...
export default function PropostasPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        setPropostas(await fetchPropostas(50));
        setError(null);
      } catch (err) {
        ...
        <table className="w-full text-left text-sm">
          <thead className="text-slate-400">
            <tr>
              ...
              <th className="pb-2">Renda / Invest.</th>
              <th className="pb-2">Beneficios</th>
            ...
            {propostas.map((p) => (
              <tr key={p.id} className="border-t border-slate-800 text-slate-300">
                ...
                <td className="py-2 text-xs">
                  <div>{formatCurrency(p.renda)}</div>
                  <div className="text-slate-500">{formatCurrency(p.investimentos)}</div>
                </td>
                <td className="py-2">
                  {p.status === 'APPROVED' ? (
                    <BenefitBadges benefits={p.beneficios} emptyLabel="-" />
                  ) : (
